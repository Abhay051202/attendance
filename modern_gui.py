...
class FaceAttendancePro:
    def __init__(self, root):
        ...
        self.camera_source = tk.StringVar(value="Camera 0")
    ...
    def setup_dashboard(self):
        ...
        tk.Label(ctrl, text="Source:", bg=COLORS['card'], fg=COLORS['text']).pack(side="left", padx=5)
        style = ttk.Style(); style.theme_use('clam')
        self.cam_combo = ttk.Combobox(ctrl, textvariable=self.camera_source, values=["Camera 0", "Camera 1", "Camera 2"], state="readonly", width=10)
        self.cam_combo.current(0); self.cam_combo.pack(side="left", padx=5)
        ...
        self.btn_cam_toggle = ModernButton(ctrl, text="START CAMERA", command=self.toggle_camera, bg=COLORS['accent'], fg="#1e1e2e", width=15)
        self.btn_cam_toggle.pack(side="right", padx=5)
        
        self.video_label = tk.Label(video_container, bg="black", text="Camera Off", fg="white")
    ...
    def toggle_camera(self):
        if self.is_running:
            self.is_running = False; self.cap.release()
            self.video_label.config(image="", text="Camera Off")
            self.cam_combo['state'] = 'readonly'; self.btn_cam_toggle.config(text="START CAMERA", bg=COLORS['accent'])
            ...
            try: idx = int(self.camera_source.get().split(" ")[1])
            except: idx = 0
            print(f"Connecting Camera {idx}...")
            self.cap = cv2.VideoCapture(idx)
            if not self.cap.isOpened(): messagebox.showerror("Error", "Cam Fail"); return
            self.is_running = True; self.cam_combo['state'] = 'disabled'; self.btn_cam_toggle.config(text="STOP CAMERA", bg=COLORS['danger'])
    ...
    def toggle_pause(self):
        ...
        if self.is_paused:
            ...
            self.video_label.config(text="PAUSED (Running in Background)")
        else:
    ...
    def update_video_loop(self):
        ...
        if ret:
            if self.current_view == "dashboard":
                annotated_frame, _, _, messages = self.processor.process_frame(frame, mark_attendance_callback=self.tracker.process_recognized_face)
                for msg in messages: self.log_list.insert(0, f"{datetime.now().strftime('%H:%M:%S')} - {msg}")
